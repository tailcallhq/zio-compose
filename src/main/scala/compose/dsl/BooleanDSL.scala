...
trait BooleanDSL[-A, +B] { self: Lambda[A, B] =>
  def &&[A1](other: A1 ~> Boolean)(implicit ev: B <:< Boolean): Lambda[A1, Boolean] =
    self and other

  def ||[A1](other: A1 ~> Boolean)(implicit ev: B <:< Boolean): Lambda[A1, Boolean] =
    self or other

  def and[A1](other: A1 ~> Boolean)(implicit ev: B <:< Boolean): Lambda[A1, Boolean] =
    unsafeMake {
  ...
  def not(implicit ev: B <:< Boolean): Lambda[A, Boolean] = unsafeMake {
    ExecutionPlan.LogicalNot(self.compile)
  }

  def or[A1](other: A1 ~> Boolean)(implicit ev: B <:< Boolean): Lambda[A1, Boolean] =
    unsafeMake {
