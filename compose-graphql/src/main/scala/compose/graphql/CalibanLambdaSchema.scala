...
object CalibanLambdaSchema {
  ...
  object Implicits {
    implicit def any2A[R, B](implicit
      schemaB: Schema[B],
      ev: CalibanSchema[R, B],
    ): CalibanSchema[R, Any ~> B] = new CalibanSchema[R, Any ~> B] {
      override def optional: Boolean = ev.optional

      override protected[this] def toType(isInput: Boolean, isSubscription: Boolean): __Type = ev
        .toType_(isInput, isSubscription)

      override def resolve(value: Any ~> B): Step[R] = Step
        .QueryStep(ZQuery.fromZIO(value.eval(()).map(ev.resolve)))
    }
    ...
    implicit def a2b[R, A, B](implicit
    ...
    ): CalibanSchema[R, A ~> B] = new CalibanSchema[R, A ~> B] {
      ...
      override def arguments: List[__InputValue] = {
        inputType.inputFields.getOrElse(handleInput(List.empty[__InputValue])(List(__InputValue(
      ...
      private def handleInput[T](onWrapped: => T)(onUnwrapped: => T): T = inputType.kind match {
      ...
      override def resolve(value: A ~> B): Step[R] = {
        FunctionStep { args =>
          ...
          handleInput(builder)(builder.fold(
            error =>
              args.get(unwrappedArgumentName)
                .fold[Either[ExecutionError, A]](Left(error))(argBuilder.build),
            Right(_),
          )).fold(
