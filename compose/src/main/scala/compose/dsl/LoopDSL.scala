...
trait LoopDSL[-A, +B] { self: A ~> B =>
  ...
  final def recurseWhile[B1 >: B <: A](cond: B1 ~> Boolean): B1 ~> B1 =
    Lambda.unsafe.attempt[B1, B1] { Recursive.RecurseWhile(self.compile, cond.compile) }

  final def repeatWhile[B1 >: B](cond: B1 ~> Boolean): A ~> B1 =
    Lambda.unsafe.attempt[A, B1](Recursive.RepeatWhile(self.compile, cond.compile))
