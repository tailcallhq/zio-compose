...
trait Interpreter {
  ...
  def evalJson[B](lmb: Any ~> B): Task[String] = evalDynamic(lmb)
    .map(res => new String(JsonCodec.encode(Schema[DynamicValue])(res).toArray))
}
...
object Interpreter {
  ...
  def eval[B](f: Any ~> B)(implicit ev: Schema[B]): Task[B] = for {
  ...
  final class InMemoryInterpreter(
    ...
    private def remote(input: DynamicValue, operation: Remote): Task[DynamicValue] =
      operation match {
        case Remote.Http => for {
            req <- effect(input.toTypedValue(Schema[Request]))
            res <- ZIO.async[Any, Nothing, Response] { cb =>
              client
                .request(req.method, req.url, req.headers, req.body) { (status, headers, body) =>
    ...
    private def eitherOne(input: DynamicValue, operation: EitherOne): Task[DynamicValue] =
      operation match {
        case EitherOne.IsLeft => input match {
    ...
    private def optional(input: DynamicValue, operation: Optional): Task[DynamicValue] =
      operation match {
        case Optional.IsEmpty => input match {
    ...
    private def arrowed(input: DynamicValue, operation: Arrow): Task[DynamicValue] = {
      operation match {
        ...
        case Arrow.ToInt => (for {
            result <- input match {
              case DynamicValue.Primitive(value, _) => ZIO.attempt(value.toString.toInt)
                  .orElseFail("Cannot convert to Int")
    ...
    private def debugger(input: DynamicValue, operation: Debugger): Task[DynamicValue] = {
      operation match {
        case Debugger.Debug(plan, name) => for {
    ...
    private def fold(input: DynamicValue, operation: Fold): Task[DynamicValue] = operation match {
      case Fold.FoldOption(isEmpty, f) => for {
          result <- input match {
      ...
      case Fold.FoldEither(left, right) => for {
          result <- input match {
    ...
    private def optical(input: DynamicValue, operation: Optical): Task[DynamicValue] = {
      operation match {
        case Optical.GetPath(path) => input match {
        ...
        case Optical.SetPath(path) => input match {
                ...
                path match {
                  ...
                  case head :: tail => values.get(head) match {
                      ...
                      case Some(v) =>
                        ...
                        else loop(tail, v.asInstanceOf[DynamicValue.Record].values, a) map {
                          value => DynamicValue.Record(id, values + (head -> value))
                        }
                    }
                }
              }
    ...
    private def scoped(input: DynamicValue, operation: Scoped): Task[DynamicValue] = {
      operation match {
        case Scoped.Set(refId, scopeId) => for {
            _ <- scope.set(scopeId, refId, input)
          } yield toDynamic(())

        case Scoped.Get(refId, scopeId, value) => for {
        ...
        case Scoped.Using(plan, scopeId) => for {
            result <- evalDynamic(plan, input)
    ...
    private def sources(input: DynamicValue, operation: Sources): Task[DynamicValue] = {
      operation match {
        ...
        case Sources.FromMap(value)  => ZIO.succeed(value.get(input) match {
            case Some(value) => DynamicValue.SomeValue(value)
            case None        => DynamicValue.NoneValue
          })
    ...
    private def toDynamic[A](a: A)(implicit schema: Schema[A]): DynamicValue = schema.toDynamic(a)

    private def tupled(input: DynamicValue, operation: Tupled): Task[DynamicValue] = {
      operation match {
        case Tupled.Arg(plan, i) => for {
            input  <- evalDynamic(plan, input)
            result <- input match {
              case DynamicValue.Tuple(left, right) => i match {
                  case 0 => ZIO.succeed(left)
                  case 1 => ZIO.succeed(right)
                  case n => ZIO.fail(new RuntimeException(
                      s"Can not extract element at index ${n} from ${input}",
                    ))
                }
